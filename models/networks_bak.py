...
def define_G(input_nc, output_nc, ngf, num_exp, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    if which_model_netG == 'resnet_9blocks':
        netG = ResnetGenerator(input_nc, output_nc, ngf, num_exp, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9, gpu_ids=gpu_ids)
...
class LSTMBlock(nn.Module):
    ...
    def init_hidden(self):
        if len(self.gpu_ids) > 0:
            self.hidden = (Variable(torch.zeros(1, 1, self.hidden_dim)).cuda(self.gpu_ids[0], async=True),
                        Variable(torch.zeros(1, 1, self.hidden_dim)).cuda(self.gpu_ids[0], async=True))
        else:
    ...
    def forward(self, input):     
        ...
        model_output, self.hidden = self.model(torch.cat((input, self.exp),2), self.hidden)
        model_output = model_output.view(-1, self.hidden_dim, 1, 1)
        return model_output
